    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_eks_cluster.main.identity[0].oidc[0].issuer, "https://", "")}:sub" = "system:serviceaccount:default:extended-api-server",
...
resource "aws_iam_policy" "app_policy" {
  #provider = aws.virginia
  name        = "appPolicy"
  description = "Policy for EKS pod to access EC2 and K8s API"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
